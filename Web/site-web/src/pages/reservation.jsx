...
import { initialState, RESET_RESERVATION, UPDATE_RESERVATION } from "../reducers/reservation-reducer";
...
function ReservationPage() {
  ...
  const handleReservation = async () => {
    ...
    if (!name || !plateau || !day || !startTime || !endTime) {
    ...
    if(startDateTime >= endDateTime) {
      setErrorMessage("L'heure de fin doit etre après l'heure de début");
      return;
    }
    ...
    try {
      ...
      if(err.message.includes("not available")) {
        setErrorMessage("Le plateau est déja réservé pour cette date");
      } else {
  ...
  const handleReset = () => {
    ...
    dispatch({ type: RESET_RESERVATION, payload : { state : initialState } });
  };
        ...
        <div className="flex-container">
          ...
          <div className="form-field">
            ...
            <select id="plateau" value={plateau} onChange={handlePlateauChange}>
              ...
              {plateaus.map((plateau) => (<option key={plateau.id} value={plateau.id}>{plateau.name}</option>))}
            </select>
          </div>
          <div className="form-field">
